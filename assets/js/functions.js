...
init: function(){
  ...
  $.get('http://localhost:5000/json/menu.json').then((Response) => {
     ...
     let retorno = _self.listMenuIntens(category);    
    return retorno;
     });
...
let vitrine = {
    ...
    listvitrine: function (items) {
        let _self = this;
        let _hmtlVitrines = items.map((item) => {
        ...
        $('.slide-vitrines').append(_hmtlVitrines);
    },
